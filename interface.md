...
--- @Description: UI will send a request to flip the chess in the given grid, return whether the user can do this. Also return new game state after this action. If the action is not allowed, just return current game state. 
...
--- @Description: just similar with the flip request. The first Int pair will be source point and the second Int pair is the destination point.
```
