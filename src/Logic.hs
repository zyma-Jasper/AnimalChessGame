module Logic (initRandomMap, sendFlipRequest,selectRequest, sendMoveRequest, isGameOver, someFunc) where
import Data.List
import Data.Matrix
import Data.Ord
...
sendMoveRequest (x0,y0) (x1,y1) g = do 
    ...
    if legal == False
        then (False, g) --- index  is illegal
            ...
            let p = (_player g)
            if ( src_v>0 && (dst_v ==0  || (dst_p/=src_p && (isBigger src_v dst_v) ) ) )--dst grid is empty(move) || dst is another players and smaller than mine
